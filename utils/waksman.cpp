 ...
 *****************************************************************************
 ...
 Implementation of interfaces for functionality for routing on an arbitrary-size
 (AS) Waksman network.
 ...
 */
size_t as_waksman_switch_output(const size_t num_packets,
 ...
 */
size_t as_waksman_switch_input(const size_t num_packets,
                               const size_t row_offset, const size_t row_idx,
                               const bool use_top) {
...
size_t as_waksman_num_columns(const size_t num_packets) {
  return (num_packets > 1 ? 2 * ZebraGRAM::log2ceil(num_packets) - 1 : 0);
}
 ...
 */
void construct_as_waksman_inner(const size_t left, const size_t right,
  ...
  if (right - left + 1 > subnetwork_width) {
    ...
    const std::vector<size_t> new_rhs_dests_top(new_rhs_dests.begin(),
                                                new_rhs_dests.begin() + d);
    const std::vector<size_t> new_rhs_dests_bottom(new_rhs_dests.begin() + d,
                                                   new_rhs_dests.end());
...
as_waksman_topology generate_as_waksman_topology(const size_t num_packets) {
  ...
  as_waksman_topology neighbors(
      width, std::vector<std::pair<size_t, size_t> >(
  ...
  construct_as_waksman_inner(0, width - 1, 0, num_packets - 1, rhs_dests,
                             neighbors);
 ...
 * column_idx.
 */
size_t as_waksman_get_canonical_row_idx(const size_t row_offset,
 ...
 */
size_t as_waksman_other_output_position(const size_t row_offset,
 ...
 * position of the other wire also connected to this switch.
 */
size_t as_waksman_other_input_position(const size_t row_offset,
                                       const size_t packet_idx) {
 ...
 * integer_permutation.
 */
void as_waksman_route_inner(const size_t left, const size_t right,
                            const size_t lo, const size_t hi,
    ...
    while (1) {
      ...
      if (route_left) {
        ...
        if (permutation.get(to_route) == hi) {
          ...
          lhs_routed[to_route - lo] = true;
          to_route =
              as_waksman_other_output_position(lo, permutation.get(to_route));
          route_left = false;
        }
      } else {
        /**
        ...
        to_route =
            as_waksman_other_input_position(lo, permutation_inv.get(to_route));
        route_left = true;
      }
...
as_waksman_routing get_as_waksman_routing(
  ...
  as_waksman_route_inner(0, width - 1, 0, num_packets - 1, permutation,
                         permutation.inverse(), routing);
  return routing;
}
  ...
  for (size_t column_idx = 0; column_idx < width; ++column_idx) {
    ...
    for (size_t packet_idx = 0; packet_idx < num_packets; ++packet_idx) {
      ...
      if (neighbors[column_idx][packet_idx].first ==
        ...
        assert((it != routing[column_idx].end()) ^
